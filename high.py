...
# Add a nice background color to buttons
button_style = """
    <style>
        .stButton > button {
        s    background-color: #FF6347;
            color: white;
            border-radius: 12px;
            height: 3em;
            width: 12em;
    ...
    if selected_movie_name:
        ...
        for i, col in enumerate(cols):
            with col:
                ...
                st.markdown(f"<a href='{urls[i]}' target='_blank' style='text-align: center; display: block; font-weight: bold;'>{names[i]}</a>", unsafe_allow_html=True)
...
st.markdown("""
    <div style='display: flex; align-items: center; justify-content: center;'>
        <div style='text-align: center;'>
            <p>Made with ❤️ by Sonu. Enjoy the movie recommendations!</p>
        </div>
        <div style='margin-left: 10px;'>
            <img src='https://m.media-amazon.com/images/I/71QGZgsRd9L.jpg' alt='Sonu' style='width: 50px; height: 50px; border-radius: 50%;'>
        </div>
    </div>
